...
spec:
  ...
  summary:
	...
	for _, test := range tests {
		t.Run(test.name, func(testing *testing.T) {
			d := Data([]byte(test.input))
			_, err := d.ParseDeployment(context.TODO(), false)
			if test.output != "" {
  ...
  summary:
	...
	for _, test := range tests {
		t.Run(test.name, func(testing *testing.T) {
			ctx := context.TODO()
			d := Data([]byte(test.input))
			deploy, _ := d.ParseDeployment(ctx, false)
			err := d.ValidateDeployment(ctx, deploy, false)
			assert.NoError(testing, err)
		})
	}
}
...
spec:
  ...
  summary:
	...
	for _, test := range tests {
		t.Run(test.name, func(testing *testing.T) {
			d := Data([]byte(test.input))
			deployment, err := d.ParseDeployment(context.TODO(), false)
			if test.output != "" {
				assert.EqualError(testing, err, test.output)
			} else {
				err = d.CheckParameter(context.TODO(), deployment, false)
				assert.EqualError(testing, err, "input parameters: [capacityInstance] should be replaced by values")

				err = d.CheckParameter(context.TODO(), deployment, true)
				assert.NoError(testing, err)
			}
	...
	for _, test := range tests {
		t.Run(test.name+" : ignoreParameter=true", func(testing *testing.T) {
			d := Data([]byte(test.input))
			err := d.CheckParameter(context.TODO(), nil, true)
			assert.NoError(testing, err)
