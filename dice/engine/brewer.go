...
func (ep *ExecutionPlan) ExecuteParallelPlan(ctx context.Context, dryRun bool, out *websocket.Conn) error {
	...
	for _, pp := range ep.ParallelPlan {
		...
		go func() {
			if err := sep.RunPlan(ctx, dryRun, wg, out); err != nil {
				log.Error(err)
			}
		}()
		wg.Add(1)
...
`
	...
	// !!! setup probe !!!
	for i, preparation := range ep.CurrentStage.Preparation {
		if strings.Contains(preparation, "dice-probe-") {
			newCmd, err := ep.ProbeWrapper(ctx, preparation, ep.CurrentStage.ProbeCommands[preparation])
			if err != nil {
				return script, err
			}
...
`
	...
	for i, prc := range ep.CurrentStage.PostRunCommands {
		if strings.Contains(ep.CurrentStage.PostRunCommands[i], "dice-probe-") {
			newCmd, err := ep.ProbeWrapper(ctx, prc, ep.CurrentStage.ProbeCommands[prc])
			if err != nil {
				return script, err
			}
	...
	SRf(out, "Generated script -  %s with success. \n", script)
	//SR(out, []byte("--BO:-------------------------------------------------"))
	//SR(out, cnt)
	//SR(out, []byte("--EO:-------------------------------------------------"))
